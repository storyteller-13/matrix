/**
 * BaseApp tests – load script in jsdom and assert init/open behavior
 */
...
describe('BaseApp', () => {
    ...
    it('open() returns early when the window element is missing', () => {
        app.window = null;
        expect(() => app.open()).not.toThrow();
    });

    it('openFallback uses flex for about-window', () => {
        document.body.innerHTML = `
            <div id="about-window" class="window about-window" style="display: none;"></div>
            <div id="about-dock-item" class="dock-item"></div>
        `;
        app = new BaseApp({ windowId: 'about-window', dockItemId: 'about-dock-item' });
        app.init();
        vi.stubGlobal('requestAnimationFrame', (cb) => { cb(); });
        app.openFallback();
        expect(app.window.style.display).toBe('flex');
    });
});
