...
class TodoApp extends BaseApp {
    ...
    render(activeCountArg) {
        ...
        // Update count
        if (todoCount) {
            const key = activeCount === 1 ? 'todo.itemLeft' : 'todo.itemsLeft';
            todoCount.textContent = window.I18n
                ? window.I18n.t(key, { count: activeCount })
        ...
        if (this.todos.length === 0) {
            todoList.innerHTML = `
                <div class="todo-empty">
                    ...
                    <div class="empty-text">${window.I18n ? window.I18n.t('todo.empty') : 'no tasks yet'}</div>
                </div>
            `;
            return;
        }
        ...
        todoList.innerHTML = this.todos.map(todo => `
            <div class="todo-item ${todo.completed ? 'completed' : ''}" data-todo-id="${todo.id}">
                ...
                <div class="todo-text">${this.escapeHtml(this.todoLabel(todo))}</div>
            </div>
        `).join('');
    }

    todoLabel(todo) {
        if (!todo.textKey || !window.I18n) return todo.text;
        const translated = window.I18n.t(todo.textKey);
        return !translated || translated === todo.textKey ? todo.text : translated;
    }
